...
namespace internal {
    ...
    void move_tuple_impl( L& lhs, R& rhs ) {
        ...
        internal::static_if<std::integral_constant<bool, N != I + 1> {}>( []( auto& l, auto& r ) { move_tuple_impl<N, I + 1>( l, r ); } )( lhs, rhs );
    }

    template<size_t N, class L, class R>
    void move_tuple( L& lhs, R& rhs ) {
        using bool_type = std::integral_constant<bool, N != 0>;
        static_if<bool_type {}>( []( auto& l, auto& r ) { move_tuple_impl<N, 0>( l, r ); } )( lhs, rhs );
    }
...
template<>
struct type_printer<QString, void> : public text_printer {};
...
template<>
struct type_printer<std::vector<int64_t>, void> : public blob_printer {};
}
...
namespace internal {
    ...
    struct is_primary_key_insertable {
        ...
        static_assert( ( tuple_helper::tuple_contains_type<primary_key_t<>, constraints_type>::value ), "an unexpected type was passed" );
...
namespace internal {
    ...
    template<class T>
    struct is_field_member_pointer<T, typename std::enable_if<std::is_member_pointer<T>::value && !std::is_member_function_pointer<T>::value>::type>
        : std::true_type {};
}
}
...
namespace internal {
    ...
    struct column_t : column_base {
        ...
        column_t( std::string name_, member_pointer_t member_pointer_, getter_type getter_, setter_type setter_, constraints_type constraints_ )
    ...
    namespace sfinae {
        ...
        struct is_column_with_insertable_primary_key<
            column_t<O, T, Op...>,
            typename std::enable_if<( tuple_helper::tuple_contains_type<primary_key_t<>, typename column_t<O, T, Op...>::constraints_type>::value )>::type> {
 ...
 */
template<class O, class T, typename = typename std::enable_if<!std::is_member_function_pointer<T O::*>::value>::type, class... Op>
internal::column_t<O, T, const T& (O::*)() const, void ( O::* )( T ), Op...> make_column( const std::string& name, T O::*m, Op... constraints ) {
    static_assert( internal::template constraints_size<Op...>::value == std::tuple_size<std::tuple<Op...>>::value, "Incorrect constraints pack" );
    static_assert( internal::is_field_member_pointer<T O::*>::value, "second argument expected as a member field pointer, not member function pointer" );
 ...
 */
template<class G, class S, typename = typename std::enable_if<internal::is_getter<G>::value>::type,
         typename = typename std::enable_if<internal::is_setter<S>::value>::type, class... Op>
internal::column_t<typename internal::setter_traits<S>::object_type, typename internal::setter_traits<S>::field_type, G, S, Op...>
...
template<>
struct field_printer<QString, void> {
...
template<>
struct field_printer<std::vector<int64_t>, void> {
    std::string operator()( const std::vector<int64_t>& t ) const {
        std::stringstream ss;
        ss << std::hex;
        for( auto c: t ) {
            ss << c;
...
template<class T, class O>
typename std::enable_if<!internal::is_offset<T>::value, internal::limit_t<T, true, true, O>>::type limit( O off, T lim ) {
 ...
 */
template<class F, class R, typename = typename std::enable_if<internal::is_base_of_template<F, internal::built_in_function_t>::value>::type>
 ...
 *  Example:
 *
 *  auto rows = storage.select(sqlite_orm::radians<std::optional<double>>(&Triangle::cornerAInDegrees));   //  decltype(rows) is
...
internal::built_in_function_t<std::string, internal::json_insert_string, X, Args...> json_insert( X x, Args... args ) {
    static_assert( std::tuple_size<std::tuple<Args...>>::value % 2 == 0, "number of arguments in json_insert must be odd" );
    return { std::tuple<X, Args...> { std::forward<X>( x ), std::forward<Args>( args )... } };
}

template<class X, class... Args>
internal::built_in_function_t<std::string, internal::json_replace_string, X, Args...> json_replace( X x, Args... args ) {
    static_assert( std::tuple_size<std::tuple<Args...>>::value % 2 == 0, "number of arguments in json_replace must be odd" );
    return { std::tuple<X, Args...> { std::forward<X>( x ), std::forward<Args>( args )... } };
}

template<class X, class... Args>
internal::built_in_function_t<std::string, internal::json_set_string, X, Args...> json_set( X x, Args... args ) {
    static_assert( std::tuple_size<std::tuple<Args...>>::value % 2 == 0, "number of arguments in json_set must be odd" );
    return { std::tuple<X, Args...> { std::forward<X>( x ), std::forward<Args>( args )... } };
}

template<class... Args>
internal::built_in_function_t<std::string, internal::json_object_string, Args...> json_object( Args... args ) {
    static_assert( std::tuple_size<std::tuple<Args...>>::value % 2 == 0, "number of arguments in json_object must be even" );
    return { std::tuple<Args...> { std::forward<Args>( args )... } };
}
...
template<class X, class... Args>
internal::built_in_function_t<std::string, internal::json_remove_string, X, Args...> json_remove( X x, Args... args ) {
    return { std::tuple<X, Args...> { std::forward<X>( x ), std::forward<Args>( args )... } };
}
...
struct table_info {
    ...
    table_info( decltype( cid ) cid_, decltype( name ) name_, decltype( type ) type_, decltype( notnull ) notnull_, decltype( dflt_value ) dflt_value_,
                decltype( pk ) pk_ )
        : cid( cid_ )
        , name( move( name_ ) )
        , type( move( type_ ) )
...
struct statement_binder<QString> {
    ...
    int bind( sqlite3_stmt* stmt, int index, const QString& value ) const {
        return statement_binder<std::string>().bind( stmt, index, value.toStdString() );
    }

    void result( sqlite3_context* context, const QString& value ) const {
        return statement_binder<std::string>().result( context, value.toStdString() );
    }
};
...
/**
 *  Specialization for optional type (std::vector<int64_t>).
 */
template<>
struct statement_binder<std::vector<int64_t>, void> {
    int bind( sqlite3_stmt* stmt, int index, const std::vector<int64_t>& value ) const {
        if( value.size() ) {
            return sqlite3_bind_blob( stmt, index, (const void*)&value.front(), int( value.size() * sizeof( int64_t ) ), SQLITE_TRANSIENT );
        } else {
            return sqlite3_bind_blob( stmt, index, "", 0, SQLITE_TRANSIENT );
        }
    }

    void result( sqlite3_context* context, const std::vector<int64_t>& value ) const {
        if( value.size() ) {
            sqlite3_result_blob( context, (const void*)&value.front(), int( value.size() * sizeof( int64_t ) ), nullptr );
        } else {
            sqlite3_result_blob( context, "", 0, nullptr );
        }
    }
 ...
 */
template<>
struct row_extractor<QString, void> {
    ...
    QString extract( sqlite3_value* value ) const {
        if( auto cStr = (const char*)sqlite3_value_text( value ) ) {
            return cStr;
        } else {
            return {};
...
/**
 *  Specialization for std::vector<int64_t>.
 */
template<>
struct row_extractor<std::vector<int64_t>> {
    std::vector<int64_t> extract( const char* row_value ) const {
        if( row_value ) {
            auto len = ::strlen( row_value );
            return this->go( static_cast<const int64_t*>( (void*)row_value ), len );
        } else {
            return {};
        }
    }

    std::vector<int64_t> extract( sqlite3_stmt* stmt, int columnIndex ) const {
        auto bytes = static_cast<const int64_t*>( sqlite3_column_blob( stmt, columnIndex ) );
        auto len = static_cast<size_t>( sqlite3_column_bytes( stmt, columnIndex ) );
        return this->go( bytes, len );
    }

    std::vector<int64_t> extract( sqlite3_value* value ) const {
        auto bytes = static_cast<const int64_t*>( sqlite3_value_blob( value ) );
        auto len = static_cast<size_t>( sqlite3_value_bytes( value ) );
        return this->go( bytes, len );
    }

protected:
    std::vector<int64_t> go( const int64_t* bytes, size_t len ) const {
        if( len ) {
            std::vector<int64_t> res;
            res.reserve( len / sizeof( int64_t ) );
            std::copy( bytes, bytes + len / sizeof( int64_t ), std::back_inserter( res ) );
            return res;
        } else {
            return {};
        }
...
template<class... Cols>
internal::index_t<typename internal::indexed_column_maker<Cols>::type...> make_index( const std::string& name, Cols... cols ) {
    return { name, false, std::make_tuple( internal::make_indexed_column( cols )... ) };
}

template<class... Cols>
internal::index_t<typename internal::indexed_column_maker<Cols>::type...> make_unique_index( const std::string& name, Cols... cols ) {
...
namespace internal {
    ...
    namespace storage_traits {
        ...
        template<class S, class T>
        struct storage_columns_count_impl<S, T, typename std::enable_if<std::is_same<T, typename S::table_type::object_type>::value>::type>
        ...
        template<class A, class B, class O>
        struct column_fk_references<foreign_key_t<A, B>, O, typename std::enable_if<std::is_same<O, typename foreign_key_t<A, B>::target_type>::value>::type> {
...
namespace internal {
    ...
    struct function_base {
        using func_call = std::function<void( sqlite3_context* context, void* functionPointer, int argsCount, sqlite3_value** values )>;
        ...
        function_base( decltype( name ) name_, decltype( argumentsCount ) argumentsCount_, decltype( create ) create_, decltype( destroy ) destroy_ )
            : name( move( name_ ) )
    ...
    struct scalar_function_t : function_base {
        ...
        scalar_function_t( decltype( name ) name_, int argumentsCount_, decltype( create ) create_, decltype( run ) run_, decltype( destroy ) destroy_ )
    ...
    struct aggregate_function_t : function_base {
        ...
        aggregate_function_t( decltype( name ) name_, int argumentsCount_, decltype( create ) create_, decltype( step ) step_, decltype( finalCall ) finalCall_,
...
internal::function_call<F, Args...> func( Args... args ) {
    ...
    static_assert( ( argsCount == functionArgsCount && !std::is_same<function_args_tuple, std::tuple<arg_values>>::value ) ||
...
namespace internal {
    ...
    struct table_template : table_base<_without_rowid> {
        ...
        std::vector<std::string> composite_key_columns_names() const {
            std::vector<std::string> res;
            this->for_each_primary_key( [ this, &res ]( auto& c ) { res = this->composite_key_columns_names( c ); } );
            return res;
        }

        std::vector<std::string> primary_key_column_names() const {
            std::vector<std::string> res;
            this->for_each_column_with<primary_key_t<>>( [ &res ]( auto& c ) { res.push_back( c.name ); } );
            if( !res.size() ) {
         ...
         */
        template<class G>
        const std::string* find_column_name( G getter, typename std::enable_if<is_getter<G>::value>::type* = nullptr ) const {
            const std::string* res = nullptr;
         ...
         */
        template<class S>
        const std::string* find_column_name( S setter, typename std::enable_if<is_setter<S>::value>::type* = nullptr ) const {
            const std::string* res = nullptr;
        ...
        std::vector<table_info> get_table_info() const {
            ...
            this->for_each_column( [ &res ]( auto& col ) {
                ...
                table_info i {
                    -1, col.name, type_printer<field_type>().print(), col.not_null(), dft, col.template has<primary_key_t<>>(),
                };
                res.emplace_back( i );
            } );
            ...
            for( size_t i = 0; i < compositeKeyColumnNames.size(); ++i ) {
                ...
                auto it = std::find_if( res.begin(), res.end(), [ &columnName ]( const table_info& ti ) { return ti.name == columnName; } );
                if( it != res.end() ) {
...
namespace internal {
    ...
    struct storage_impl_base {
        ...
        static bool calculate_remove_add_columns( std::vector<table_info*>& columnsToAdd, std::vector<table_info>& storageTableInfo,
            ...
            for( size_t storageColumnInfoIndex = 0; storageColumnInfoIndex < storageTableInfo.size(); ++storageColumnInfoIndex ) {
                ...
                auto dbColumnInfoIt = std::find_if( dbTableInfo.begin(), dbTableInfo.end(), [ &columnName ]( auto& ti ) { return ti.name == columnName; } );
                if( dbColumnInfoIt != dbTableInfo.end() ) {
                    auto& dbColumnInfo = *dbColumnInfoIt;
                    auto columnsAreEqual = dbColumnInfo.name == storageColumnInfo.name && dbColumnInfo.notnull == storageColumnInfo.notnull &&
                        ( dbColumnInfo.dflt_value.length() > 0 ) == ( storageColumnInfo.dflt_value.length() > 0 ) && dbColumnInfo.pk == storageColumnInfo.pk;
                    if( !columnsAreEqual ) {
                        notEqual = true;
                        break;
                    }
    ...
    struct storage_impl<H, Ts...> : public storage_impl<Ts...> {
        ...
        const std::string* column_name( F O::*m, typename std::enable_if<std::is_same<O, HH>::value>::type* = nullptr ) const {
            return this->table.find_column_name( m );
        }
        ...
        const std::string* column_name( const column_pointer<T, F>& c, typename std::enable_if<std::is_same<T, HH>::value>::type* = nullptr ) const {
         ...
         */
        void copy_table( sqlite3* db, const std::string& name, const std::vector<table_info*>& columnsToIgnore ) const {
            std::stringstream ss;
            std::vector<std::string> columnNames;
            this->table.for_each_column( [ &columnNames, &columnsToIgnore ]( auto& c ) {
                auto& columnName = c.name;
                auto columnToIgnoreIt = std::find_if( columnsToIgnore.begin(), columnsToIgnore.end(),
                                                      [ &columnName ]( auto tableInfoPointer ) { return columnName == tableInfoPointer->name; } );
 ...
 *  ```c++
 *  auto statement = storage.prepare(insert(or_ignore(), into<Singer>(), columns(&Singer::name), values(std::make_tuple("Sofia Reyes"),
 * std::make_tuple("Kungs")))); auto statement2 = storage.prepare(insert(or_rollback(), into<Singer>(), default_values())); auto statement3 =
 * storage.prepare(insert(or_abort(), into<User>, columns(&User::id, &User::name), values(std::make_tuple(5, "Little Mix"))));
 *  ```
 */
template<class... Args>
 ...
 */
template<class It>
internal::replace_range_t<It, internal::default_transformer, typename std::iterator_traits<It>::value_type> replace_range( It from, It to ) {
    return { { std::move( from ), std::move( to ) } };
}

/**
 *  Create an replace range statement with explicit transformer. Transformer is used to apply containers with no strict objects with other kind of objects like
 * pointers, optionals or whatever.
 *  @example
 *  ```
 ...
 */
template<class It>
internal::insert_range_t<It, internal::default_transformer, typename std::iterator_traits<It>::value_type> insert_range( It from, It to ) {
    return { { std::move( from ), std::move( to ) }, internal::default_transformer {} };
}

/**
 *  Create an insert range statement with explicit transformer. Transformer is used to apply containers with no strict objects with other kind of objects like
 * pointers, optionals or whatever.
 *  @example
 *  ```
 *  std::vector<std::unique_ptr<User>> userPointers;
...
namespace internal {
     ...
     */
    struct backup_t {
        backup_t( connection_ref to_, const std::string& zDestName, connection_ref from_, const std::string& zSourceName,
...
namespace internal {
    ...
    struct storage_base {
        ...
        void create_collation( const std::string& name, collating_function f ) {
            ...
            if( this->connection->retain_count() > 0 ) {
                auto db = this->connection->get();
                auto resultCode = sqlite3_create_collation( db, name.c_str(), SQLITE_UTF8, functionPointer, functionExists ? collate_callback : nullptr );
    ...
    protected:
        ...
        void delete_function_impl( const std::string& name, std::vector<std::unique_ptr<function_base>>& functionsVector ) const {
            auto it = find_if( functionsVector.begin(), functionsVector.end(), [ &name ]( auto& functionPointer ) { return functionPointer->name == name; } );
            if( it != functionsVector.end() ) {
                ...
                if( this->connection->retain_count() > 0 ) {
                    auto db = this->connection->get();
                    auto resultCode = sqlite3_create_function_v2( db, name.c_str(), 0, SQLITE_UTF8, nullptr, nullptr, nullptr, nullptr, nullptr );
                    if( resultCode != SQLITE_OK ) {
                        throw std::system_error( std::error_code( sqlite3_errcode( db ), get_sqlite_error_category() ), sqlite3_errmsg( db ) );
                    }
                }
        ...
        void try_to_create_function( sqlite3* db, scalar_function_t& function ) {
            auto resultCode = sqlite3_create_function_v2( db, function.name.c_str(), function.argumentsCount, SQLITE_UTF8, &function, scalar_function_callback,
        ...
        void try_to_create_function( sqlite3* db, aggregate_function_t& function ) {
            auto resultCode = sqlite3_create_function( db, function.name.c_str(), function.argumentsCount, SQLITE_UTF8, &function, nullptr,
                                                       aggregate_function_step_callback, aggregate_function_final_callback );
            if( resultCode != SQLITE_OK ) {
                throw std::system_error( std::error_code( resultCode, get_sqlite_error_category() ), sqlite3_errstr( resultCode ) );
            }
        }

        static void aggregate_function_step_callback( sqlite3_context* context, int argsCount, sqlite3_value** values ) {
        ...
        static void scalar_function_callback( sqlite3_context* context, int argsCount, sqlite3_value** values ) {
            ...
            std::unique_ptr<int, void ( * )( int* )> callablePointer( functionPointer->create(), functionPointer->destroy );
        ...
        static int foreign_keys_count( T& storageImpl ) {
            auto res = 0;
            storageImpl.for_each( [ &res ]( auto& impl ) { res += impl.foreign_keys_count(); } );
            return res;
        }
...
namespace internal {
    ...
    struct statement_serializator<upsert_clause<std::tuple<TargetArgs...>, std::tuple<ActionsArgs...>>, void> {
        ...
        std::string operator()( const statement_type& statement, const C& context ) const {
            ...
            if( std::tuple_size<typename statement_type::actions_tuple>::value == 0 ) {
                ...
                iterate_tuple( statement.actions, [ &ss, &updateContext ]( auto& value ) { ss << ' ' << serialize( value, updateContext ); } );
            }
            return ss.str();
        }
    };
    ...
    struct statement_serializator<built_in_function_t<R, S, Args...>, void> {
        ...
        std::string operator()( const statement_type& statement, const C& context ) const {
            ...
            iterate_tuple( statement.args, [ &args, &context ]( auto& v ) { args.push_back( serialize( v, context ) ); } );
    ...
    template<class T>
    struct statement_serializator<T, typename std::enable_if<is_base_of_template<T, compound_operator>::value>::type> {
        using statement_type = T;
    ...
    struct statement_serializator<simple_case_t<R, T, E, Args...>, void> {
        ...
        std::string operator()( const statement_type& c, const C& context ) const {
            std::stringstream ss;
            ss << "CASE ";
            c.case_expression.apply( [ &ss, context ]( auto& c_ ) { ss << serialize( c_, context ) << " "; } );
            iterate_tuple( c.args, [ &ss, context ]( auto& pair ) {
                ss << "WHEN " << serialize( pair.first, context ) << " ";
                ss << "THEN " << serialize( pair.second, context ) << " ";
            } );
            c.else_expression.apply( [ &ss, context ]( auto& el ) { ss << "ELSE " << serialize( el, context ) << " "; } );
            ss << "END";
            return ss.str();
        }
    };
    ...
    template<class T>
    struct statement_serializator<T, typename std::enable_if<is_base_of_template<T, binary_condition>::value>::type> {
        using statement_type = T;
    ...
    struct statement_serializator<in_t<L, Args...>, void> {
        ...
        std::string operator()( const statement_type& c, const C& context ) const {
            ...
            using args_type = std::tuple<Args...>;
            args.reserve( std::tuple_size<args_type>::value );
            iterate_tuple( c.argument, [ &args, &context ]( auto& v ) { args.push_back( serialize( v, context ) ); } );
            for( size_t i = 0; i < args.size(); ++i ) {
                ss << args[ i ];
                if( i < args.size() - 1 ) {
                    ss << ", ";
    ...
    struct statement_serializator<like_t<A, T, E>, void> {
        ...
        std::string operator()( const statement_type& c, const C& context ) const {
            ...
            c.arg3.apply( [ &ss, &context ]( auto& value ) { ss << " ESCAPE " << serialize( value, context ); } );
            return ss.str();
        }
    };
    ...
    struct statement_serializator<column_t<O, T, G, S, Op...>, void> {
        ...
        std::string operator()( const statement_type& c, const C& context ) const {
            ...
            ss << type_printer<field_type>().print() << " ";
            {
                ...
                int tupleIndex = 0;
                iterate_tuple( c.constraints, [ &constraintsStrings, &primaryKeyIndex, &autoincrementIndex, &tupleIndex, &context ]( auto& v ) {
    ...
    struct statement_serializator<remove_all_t<T, Args...>, void> {
        ...
        std::string operator()( const statement_type& rem, const C& context ) const {
            ...
            iterate_tuple( rem.conditions, [ &context, &ss ]( auto& v ) { ss << serialize( v, context ); } );
            return ss.str();
        }
    };
    ...
    struct statement_serializator<set_t<Args...>, void> {
        ...
        std::string operator()( const statement_type& statement, const C& context ) const {
            ...
            iterate_tuple( statement.assigns, [ &ss, &context, &leftContext, &assignIndex, assignsCount ]( auto& value ) {
    ...
    struct statement_serializator<update_all_t<set_t<Args...>, Wargs...>, void> {
        ...
        std::string operator()( const statement_type& upd, const C& context ) const {
            std::stringstream ss;
            ss << "UPDATE ";
            table_name_collector collector( [ &context ]( std::type_index ti ) { return context.impl.find_table_name( ti ); } );
            ...
            if( !collector.table_names.empty() ) {
                if( collector.table_names.size() == 1 ) {
                    ...
                    iterate_tuple( upd.conditions, [ &context, &ss ]( auto& v ) { ss << serialize( v, context ); } );
                    return ss.str();
                } else {
    ...
    template<class T>
    struct statement_serializator<T, typename std::enable_if<is_insert_raw<T>::value || is_replace_raw<T>::value>::type> {
        using statement_type = T;
    ...
    std::string serialize_get_all_impl( const T& get, const C& context ) {
        ...
        collector.table_names.insert( std::make_pair( context.impl.find_table_name( typeid( primary_type ) ), std::string {} ) );
        ...
        ss << "FROM ";
        std::vector<std::pair<std::string, std::string>> tableNames( collector.table_names.begin(), collector.table_names.end() );
        ...
        iterate_tuple( get.conditions, [ &context, &ss ]( auto& v ) { ss << serialize( v, context ); } );
        return ss.str();
    }
    ...
    struct statement_serializator<select_t<T, Args...>, void> {
        ...
        std::string operator()( const statement_type& sel, const C& context ) const {
            ...
            iterate_tuple( sel.conditions, [ &context, &ss ]( auto& v ) { ss << ' ' << serialize( v, context ); } );
    ...
    struct statement_serializator<index_t<Cols...>, void> {
        ...
        std::string operator()( const statement_type& statement, const C& context ) const {
            ...
            ss << "INDEX IF NOT EXISTS '" << statement.name << "' ON '" << context.impl.find_table_name( typeid( indexed_type ) ) << "' (";
            std::vector<std::string> columnNames;
    ...
    struct statement_serializator<limit_t<T, HO, OI, O>, void> {
        ...
        std::string operator()( const statement_type& limt, const C& context ) const {
            ...
            if( HO ) {
                if( OI ) {
                    limt.off.apply( [ &newContext, &ss ]( auto& value ) { ss << serialize( value, newContext ); } );
                    ss << ", ";
...
namespace internal {
    ...
    public:
         ...
         *  @note If you need to return the result in a different container type then use a different `get_all` function overload `get_all<User,
         * std::list<User>>`
         ...
         *  @example: storage.get_all<User, std::list<User>>(where(like(&User::name, "N%")), order_by(&User::id)); - SELECT * FROM users WHERE name LIKE 'N%'
         * ORDER BY id
         */
        template<class O, class R, class... Args>
        auto get_all( Args&&... args ) {
         ...
         *  @note If you need to return the result in a different container type then use a different `get_all_pointer` function overload `get_all_pointer<User,
         * std::list<User>>`
         ...
         */
        template<class O, class... Ids>
        bool get( std::shared_ptr<O> object, Ids... ids ) {
            this->assert_mapped_type<O>();
            auto statement = this->prepare( sqlite_orm::get_pointer<O>( std::forward<Ids>( ids )... ) );
            return this->execute( object, statement );
        }
        ...
        template<class T>
        typename std::enable_if<is_prepared_statement<T>::value, std::string>::type dump( const T& preparedStatement ) const {
            using context_t = serializator_context<impl_type>;
            context_t context { this->impl };
         ...
         */
        template<class O>
        typename std::enable_if<storage_traits::type_is_mapped<self, O>::value, std::string>::type dump( const O& o ) {
            auto& tImpl = this->get_impl<O>();
            std::stringstream ss;
            ss << "{ ";
        ...
        template<class O>
        void replace( std::shared_ptr<O> o ) {
            this->assert_mapped_type<O>();
            O& ref = *o.get();
            auto statement = this->prepare( sqlite_orm::replace( std::ref( ref ) ) );
            this->execute( statement );
        }
    ...
    protected:
        ...
        sync_schema_result schema_status( const storage_impl<TTable<Cs...>, Tss...>& tImpl, sqlite3* db, bool preserve ) {
        ...
        template<template<class...> class TTable, class... Tss, class... Cs>
        sync_schema_result sync_table( const storage_impl<TTable<Cs...>, Tss...>& tImpl, sqlite3* db, bool preserve ) {
            auto res = sync_schema_result::already_in_sync;
            ...
            if( schema_stat != decltype( schema_stat )::already_in_sync ) {
                if( schema_stat == decltype( schema_stat )::new_table_created ) {
                    ...
                    if( schema_stat == sync_schema_result::old_columns_removed || schema_stat == sync_schema_result::new_columns_added ||
    ...
    public:
        ...
        template<class T, typename std::enable_if<( is_replace_range<T>::value || is_replace<T>::value )>::type* = nullptr>
        void execute( const prepared_statement_t<T>& statement ) {
            ...
            static_if<is_replace_range<T> {}>(
                [ &processObject ]( auto& statement ) {
                    ...
                    std::for_each( ///
                        statement.t.range.first, statement.t.range.second, [ &processObject, &transformer ]( auto& object ) {
        ...
        template<class T, typename std::enable_if<( is_insert_range<T>::value || is_insert<T>::value )>::type* = nullptr>
        ...
        template<class T, class... Ids>
        bool execute( std::shared_ptr<T> res,  const prepared_statement_t<get_pointer_t<T, Ids...>>& statement ) {
            auto& tImpl = this->get_impl<T>();
            auto con = this->get_connection();
            auto db = con.get();
            auto stmt = statement.stmt;
        ...
        std::vector<R> execute( const prepared_statement_t<select_t<T, Args...>>& statement ) {
            ...
            do {
                ...
                case SQLITE_ROW: {
                    ...
                    row_extractor_builder<R, storage_traits::type_is_mapped<self, R>::value, table_info_t> builder;
    ...
    internal::iterate_ast( statement.t, [ &result, &index ]( auto& node ) {
        ...
        if( index == N ) {
            internal::static_if<std::is_same<result_tupe, node_type> {}>(
                []( auto& r, auto& n ) { r = const_cast<typename std::remove_reference<decltype( r )>::type>( &n ); } )( result, node );
        }
    } );
